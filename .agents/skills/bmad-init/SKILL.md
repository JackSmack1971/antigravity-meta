...
- `target_module` — Target module.yaml questions (when module setup needed, discovered from `--skill-path` or `.agents/bmad/{module}/`)
...
```json
{
  "core": {
    ...
    "output_folder": ".agents/bmad-output"
  },
  "bmb": {
    "bmad_builder_output_folder": ".agents/bmad-output/skills",
    "bmad_builder_reports": ".agents/bmad-output/reports"
  }
}
```
...
The script:
- Creates `.agents/bmad/core/config.yaml` with core values (if core answers provided)
- Creates `.agents/bmad/{module}/config.yaml` with core values + module values (result-expanded)
