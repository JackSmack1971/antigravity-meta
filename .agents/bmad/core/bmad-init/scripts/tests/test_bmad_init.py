...
class TestFindProjectRoot(unittest.TestCase):
    ...
    def test_finds.agents/bmad_folder(self):
        temp_dir = tempfile.mkdtemp()
        try:
            (Path(temp_dir) / '.agents/bmad').mkdir()
    ...
    def test_llm_provided_with.agents/bmad(self):
        temp_dir = tempfile.mkdtemp()
        try:
            (Path(temp_dir) / '.agents/bmad').mkdir()
    ...
    def test_llm_provided_without.agents/bmad_still_returns_dir(self):
        """First-run case: LLM provides path but .agents/bmad doesn't exist yet."""
        temp_dir = tempfile.mkdtemp()
        try:
            result = find_project_root(llm_provided=temp_dir)
            self.assertEqual(result.resolve(), Path(temp_dir).resolve())
...
class TestExpandTemplate(unittest.TestCase):
    ...
    def test_multiple_placeholders(self):
        result = expand_template(
            ...
            {'output_folder': '.agents/bmad-output', 'project-root': '/test'}
        )
        self.assertEqual(result, '.agents/bmad-output/planning')
...
class TestApplyResultTemplate(unittest.TestCase):
    ...
    def test_with_result_template(self):
        ...
        result = apply_result_template(var_def, '.agents/bmad-output', {'project-root': '/test'})
        self.assertEqual(result, '/test/.agents/bmad-output')
...
class TestFindTargetModuleYaml(unittest.TestCase):
    ...
    def test_finds_in.agents/bmad_module_dir(self):
        module_dir = self.project_root / '.agents/bmad' / 'mymod'
    ...
    def test_skill_path_takes_priority(self):
        """Skill assets module.yaml takes priority over .agents/bmad/{module}/."""
        skill_path = self.project_root / 'skills' / 'test-skill'
        assets = skill_path / 'assets'
        assets.mkdir(parents=True)
        (assets / 'module.yaml').write_text('code: test\nname: from-skill\n')

        module_dir = self.project_root / '.agents/bmad' / 'test'
        module_dir.mkdir(parents=True)
...
class TestLoadModuleConfig(unittest.TestCase):
    ...
    def setUp(self):
        self.temp_dir = tempfile.mkdtemp()
        self.project_root = Path(self.temp_dir)
        bmad_core = self.project_root / '.agents/bmad' / 'core'
        bmad_core.mkdir(parents=True)
        (bmad_core / 'config.yaml').write_text(
            'user_name: TestUser\n'
            'communication_language: English\n'
            'document_output_language: English\n'
            'output_folder: "{project-root}/.agents/bmad-output"\n'
        )
        bmad_bmb = self.project_root / '.agents/bmad' / 'bmb'
        ...
        (bmad_bmb / 'config.yaml').write_text(
            ...
            'document_output_language: English\n'
            'output_folder: "{project-root}/.agents/bmad-output"\n'
            'bmad_builder_output_folder: "{project-root}/.agents/bmad-output/skills"\n'
            'bmad_builder_reports: "{project-root}/.agents/bmad-output/reports"\n'
        )
