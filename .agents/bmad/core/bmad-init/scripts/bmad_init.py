...
Config files (flat YAML per module):
  - .agents/bmad/core/config.yaml (core settings — user_name, language, output_folder, etc.)
  - .agents/bmad/{module}/config.yaml (module settings + core values merged in)
...
def find_project_root(llm_provided=None):
    """
    Find project root by looking for .agents/bmad folder.
    ...
    if llm_provided:
        ...
        if (candidate / '.agents/bmad').exists():
            return candidate
        # First run — .agents/bmad won't exist yet but LLM path is still valid
        if candidate.is_dir():
            return candidate
    ...
    for start_dir in [Path.cwd(), Path(__file__).resolve().parent]:
        ...
        while current_dir != current_dir.parent:
            if (current_dir / '.agents/bmad').exists():
                return current_dir
...
def find_target_module_yaml(module_code, project_root, skill_path=None):
    ...
    Search order:
      ...
      3. .agents/bmad/{module_code}/module.yaml (installed module location)
    """
    search_paths = []
    ...
    if project_root and module_code:
        search_paths.append(Path(project_root) / '.agents/bmad' / module_code / 'module.yaml')
...
def load_module_config(module_code, project_root):
    """Load config for a specific module from .agents/bmad/{module}/config.yaml."""
    config_path = Path(project_root) / '.agents/bmad' / module_code / 'config.yaml'
...
def apply_result_template(var_def, raw_value, context):
    ...
    E.g., result: "{project-root}/{value}" with value=".agents/bmad-output"
    becomes "/Users/foo/project/.agents/bmad-output"
    """
...
def cmd_load(args):
    ...
    if not project_root:
        print(json.dumps({'error': 'Project root not found (.agents/bmad folder not detected)'}),
              file=sys.stderr)
        sys.exit(1)
...
def cmd_write(args):
    ...
    if core_answers_raw and core_def:
        ...
        # Write core config
        core_dir = project_root / '.agents/bmad' / 'core'
    ...
    for module_code, module_answers_raw in answers.items():
        ...
        # Write module config
        module_dir = project_root / '.agents/bmad' / module_code
